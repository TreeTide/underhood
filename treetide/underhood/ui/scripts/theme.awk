...
function splitpanes_to_css(re, splitpanesPostfix, pre) {
  res = match($0, re, parts);
  if (res) {
    print ".uh-s-"theme" .splitpanes--"splitpanesPostfix" > .splitpanes__splitter { "pre""parts[1]" !important }";
  }
  return res;
}
...
$0 ~ theme_style" *div.CodeMirror-selected" {
  ...
  splitpanes_to_css(bg_regexp, "vertical", "background: ");
  splitpanes_to_css(bg_regexp, "horizontal", "background: ");
}

$0 ~ theme_style" *.CodeMirror-gutters {" {
  splitpanes_to_css(bg_regexp, "vertical", "border-left: 2px solid ");
