...
type MultibyteChar = (CharOffset, CharBytes)
...
data LineCol = LineCol !Int !CharOffset
    deriving (Eq, Ord, Show)

byteOffsetToLineCol :: OffsetTable -> ByteOffset -> Maybe LineCol
byteOffsetToLineCol OffsetTable{..} pos = do
    -- TODO(robinp): some assumption here that the byte is always in the range.
    -- Could check.
    let lineIndex = case UV.findIndex (\(_, byteOffset) -> byteOffset > pos) lineOffsets of
          Nothing -> UV.length lineOffsets - 1
          Just i -> i - 1
    let lineStartByteOffset = snd (lineOffsets UV.! lineIndex)
    (_charCount, multis) <- lineCharInfo V.!? lineIndex
    let inLineByteOffset = pos - lineStartByteOffset
    column <- go inLineByteOffset (LineState { lsLastCharEnd = 0, lsBytes = 0 }) (UV.toList multis)
    pure (LineCol lineIndex column)
  where
    go :: ByteOffset -> LineState -> [MultibyteChar] -> Maybe Int
    go target LineState{..} ms = case ms of
        -- TODO(robinp): check line's charCount as bound.
        [] -> pure (lsLastCharEnd + target - lsBytes)
        (currentCharOffset, mbs):rest ->
            let deltaBytes = currentCharOffset - lsLastCharEnd
                bytes = lsBytes + deltaBytes
                diff = target - bytes
            in if diff <= 0 then pure (currentCharOffset + diff)
                            else go target LineState
                                    { lsLastCharEnd = currentCharOffset + 1
                                    , lsBytes = bytes + mbs
                                    }
                                    rest

data LineState = LineState
    { lsLastCharEnd :: !CharOffset
    , lsBytes :: !ByteOffset
    }
